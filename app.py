...
def main():
    ...
    try:
        ...
        # LIMPIEZA AUTOMÁTICA
        df.columns = df.columns.str.strip() # Quita espacios en nombres de columnas
        
        # Mapeo de columnas necesarias
        cols_necesarias = ['Empresa', 'Nicho', 'Producto / Servicio', 'Factor de Éxito', 'Peso (%)', 'Calificación (1-5)']
    ...
    prods_disp = df_n['Producto / Servicio'].unique()
    prods_sel = st.multiselect("3. Compare Productos/Servicios", prods_disp)
    ...
    if prods_sel:
        ...
        for p in prods_sel:
            ...
            res_dict[p] = (c * w / 100).sum()

        fig.update_layout(
            polar=dict(radialaxis=dict(visible=True, range=[0, 5])),
            title=f"Ajuste de Portafolio para el Nicho: {nicho_sel}"
        )
        ...
        st.info("Seleccione productos para visualizar el análisis.")
